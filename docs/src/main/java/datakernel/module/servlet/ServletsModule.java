...
public final class ServletsModule extends AbstractModule {
    private static Promise<ByteBuf> renderPage(HtmlRenderer renderer, PageDao pageDao, Parser parser, Mustache page,
											   @NotNull String sector, @Nullable String doc, @Nullable String destination) {
    ...
    MutableDataSet options() {
        ...
        options.set(Parser.EXTENSIONS, asList(TablesExtension.create(), StrikethroughExtension.create()));
        return options;
    }
    ...
    @Provides
    @Named("home")
    AsyncServlet homeServlet(@Named("properties") Config config, Executor executor) {
        return StaticServlet.ofClassPath(executor, config.get("templates.path"))
                .withIndexHtml();
    }
    ...
    AsyncServlet sectorServlet(@Named("properties") Config config,
        ...
        return request -> {
            ...
            return renderPage(renderer, pageDao, mdParser, page, sector, null, null)
    ...
    AsyncServlet docServlet(@Named("properties") Config config,
        ...
        return request -> {
            ...
            String sector = request.getPathParameter("sector");
            return renderPage(renderer, pageDao, parser, page, sector, doc, destination)
                    .map(content -> HttpResponse.ok200().withBody(content));
        };
    }
}
